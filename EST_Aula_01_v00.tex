...
\begin{frame}[t,fragile=singleslide]
\frametitle{Regressão Linear e Correlação}
\vspace{0.5cm}
\begin{itemize}
...
\begin{tikzpicture}[remember picture,overlay]
\node[xshift=4cm,yshift=-2.0cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{Mult_regre.png}};
\end{tikzpicture}
\vspace{-0.5cm}
\begin{itemize}
	\item[-] Associação entre variáveis quantitativas.
...
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
\frametitle{Comparando modelos - qqplot}
\begin{lstlisting}
...
\begin{tikzpicture}[remember picture,overlay]
\node[xshift=-8cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/QQPlot_M01.png}};
\end{tikzpicture}

\begin{tikzpicture}[remember picture,overlay]
\node[xshift=8cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/QQPlot_M02.png}};
\end{tikzpicture}

\end{frame}
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
\frametitle{Comparando modelos - leverage}
\vspace{1cm}
Projeção lateral dos modelos
\vspace{1cm}
\begin{tikzpicture}[remember picture,overlay]
\node[xshift=-8cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/Leverange_M01.png}};
\end{tikzpicture}

\begin{tikzpicture}[remember picture,overlay]
\node[xshift=8cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/Leverange_M02.png}};
\end{tikzpicture}
\end{frame}
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
...
\begin{frame}[t,fragile=singleslide]
	...
	\textbf{Exercícios do livro \cite{Agresti2018}:}
	\begin{itemize}
		\item[-] Capítulo 11: %10.5, 10.13, 10.19, 10.29.
	\end{itemize}
\end{frame}
% ------------------------------------------------------------------------
...
\begin{table}[]
	...
	\begin{tabularx}{30cm}{|C{9cm} | C{9cm} | C{9cm}|}
		\hline
		\rowcolor[HTML]{C0C0C0} ANOVA faz & ANOVA não faz & Alternativa \\ \hline
...
\end{lstlisting}
\begin{tikzpicture}[remember picture,overlay]
\node[xshift=5cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=16cm]{../Rcodes/BoxPlot_NSL_00.png}};
\end{tikzpicture}
\end{frame}
...
\vspace{1cm}
\begin{tikzpicture}[remember picture,overlay]
\node[xshift=0cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=16cm]{../Rcodes/Diferenca_MannerPath_01.png}};
\end{tikzpicture}
...
\begin{frame}[t,fragile=singleslide]
	...
	\textbf{Exercícios do livro \cite{Agresti2018}:}
	\begin{itemize}
		\item[-] Capítulo 12: %10.5, 10.13, 10.19, 10.29.
	\end{itemize}
\end{frame}
% ------------------------------------------------------------------------
...
\begin{frame}[t,fragile=singleslide]
\frametitle{Associação de Variáveis quantitiaticas com categóricas.}

\begin{tikzpicture}[remember picture,overlay]
\node[xshift=6.5cm,yshift=-4.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{logistic-regression-diagram.png}};
\end{tikzpicture}

Requisitos 
\begin{itemize}
	\item[-] Observações são indepependentes.
	\item[-] A relação entre o logit $\log \left( \frac{p}{1-p}  \right)$ a as variáveis quantitativas são lineares.
	\item[-] Não tem colinearidade (rependência linear, correlação) entre as variáveis quantitativas.
	\item[-] Não pressupõe normalidade dos resíduos nem homocedasticidade.
\end{itemize}
\vspace{1.5cm}
Basicamente:
\begin{flalign*}
&\log \left( \frac{p}{1-p}  \right) = a_{1}x_{1} + a_{2}x_{2} + ... + a_{n}x_{n} + b & \\
\end{flalign*}

\lfr{\url{https://www.tibco.com/pt-br/reference-center/what-is-logistic-regression}}
\end{frame}
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
	\frametitle{Tipos}
	...
	\begin{tikzpicture}[remember picture,overlay]
	\node[xshift=0cm,yshift=0cm,opacity=1.0] at (current page.center) {\includegraphics[width=18cm]{regressao_tipos.png}};
	\end{tikzpicture}
	
	%	regressao_tipos.png
	%	https://operdata.com.br/blog/regressao-logistica-multinomial/
	\lfr{Fonte \url{https://operdata.com.br/blog/regressao-logistica-multinomial/}}
\end{frame}
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
	\frametitle{Interpretação}
	Suponha a existência de dois grupos, por exemplo um com disfunção na fala e outro de controle. Como indicado na Tabela.

	\begin{table}[]
		\centering
		\begin{tabular}{lccc}
			&         & \multicolumn{2}{c}{Número de gestos $x$} \\ \cline{3-4} 
			Grupo     & Amostras & Média         & Desvio padrão        \\ \hline
			Disfunção ($G_1$) & 20       & 5,8           & 7,9                  \\
			Controle  ($G_2$) & 20       & 12,7          & 9,4                 
		\end{tabular}
	\end{table}
	
	Suponhamos que podemos obter as curvas de densidade de probabilidade para cada grupo.
%	regressao_tipos.png
%	https://operdata.com.br/blog/regressao-logistica-multinomial/
	\lfr{O exemplo utiliza dos dados da pesquisa não publicada (por enquanto) da aluna Ana Vieira e Beatriz de Matos.}
\end{frame}
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
	\frametitle{Construído um modelo}
	
	Distribuição das ocorrências de $G_1$ e $G_2$. 
			
	\vspace{1cm}
	\begin{tikzpicture}[remember picture,overlay]
	\node[xshift=0cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=16cm]{../Rcodes/R10_Gaussianas_Gestos_Grupo_00.png}};
	\end{tikzpicture}
...
\begin{frame}[t,fragile=singleslide]
	...
	\begin{flalign*}
	&\frac{P(Y=G_1 |x)}{1 - P(Y=G_1 |x)}
	\end{flalign*}
	\vspace{1.5cm}
	e o valor logaritmico dessa proposrção seria linear
	\begin{flalign*}
	&\log \left( \frac{P(Y=G_1 |x)}{1 - P(Y=G_1 |x)}  \right) = a_{1}x_{1} + a_{2}x_{2} + ... + a_{n}x_{n} + b & \\
	\end{flalign*}
...
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
	\frametitle{Modelo no R}
	
	\begin{tikzpicture}[remember picture,overlay]
	\node[xshift=0cm,yshift=-4.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/R10_Logistica_Gestos_Grupo_00.png}};
	\end{tikzpicture}
...
lrm(formula = GRUPO ~ Ges_Total, data = df_disf)

Model Likelihood     Discrimination    Rank Discrim.    
Ratio Test           Indexes           Indexes       
Obs            40    LR chi2      6.06    R2       0.188    C       0.759    
G1            20    d.f.            1    R2(1,40) 0.119    Dxy     0.518    
G2            20    Pr(> chi2) 0.0138    R2(1,30) 0.155    gamma   0.535    
max |deriv| 1e-08                         Brier    0.214    tau-a   0.265    

Coef    S.E.   Wald Z Pr(>|Z|)
Intercept -0.8378 0.4891 -1.71  0.0867  
Ges_Total  0.0945 0.0425  2.22  0.0263 
\end{lstlisting}
...
% ------------------------------------------------------------------------
\begin{frame}[t,fragile=singleslide]
	\frametitle{Ajuste da curva ROC}
	Característica de Operação do Receptor
	
	\begin{tikzpicture}[remember picture,overlay]
	\node[xshift=0cm,yshift=-2.5cm,opacity=1.0] at (current page.center) {\includegraphics[width=14cm]{../Rcodes/R10_ROC_Gestos_Grupo_00.png}};
	\end{tikzpicture}
